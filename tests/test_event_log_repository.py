...
@pytest.fixture
def event_log_repository() -> EventLogRepository:
    db_path = Path(__file__).resolve().parent / f"event_logs_{uuid4().hex}.db"
    init_schema(db_path)

    yield EventLogRepository(str(db_path))
...
def test_filters_and_counts_events(
    ...
    recent_events = event_log_repository.select_between(
        start_timestamp="2026-06-18T10:30:00",
        end_timestamp="2026-06-18T11:30:00",
    )

    assert [event.container_name for event in recent_events] == ["worker"]
